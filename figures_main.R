...
sim_all<- readRDS("all_outputs/sim_all_out.rds")

summarised_rmse <- sim_all[mu.v.yr == 0 & term %in% c("mu.psi.yr", "p.yr") & nyr == 10][,.(rmse_vals = rmse(true_val, estimate), N = .N), 
...
summarised_rmse <- sim_all[mu.v.yr == 0 & term %in% c("mu.psi.yr", "p.yr")][,.(rmse_vals = rmse(true_val, estimate), N = .N), 
...
summarised_rmse_sim_all <- sim_all[mu.v.yr == 0 & term %in% c("mu.psi.yr", "p.yr") & eras == "eras5"][,.(rmse_vals = rmse(true_val, estimate), N = .N), 
                                                                                   by = .(visit_mod, term, eras, prop.visits.same)] %>% 
  mutate(scenario = "sim_all")

summarised_rmse_sim_all$nyr <- factor(summarised_rmse_sim_all$eras, 
                              levels = c("eras2", "eras5", "eras10"), 
                              labels = c(expression("2"~" OIs"),expression("5"~" OIs"),expression("10"~" OIs")))

summarised_rmse_sim_all$term <- factor(summarised_rmse_sim_all$term, 
                               levels=c("mu.psi.yr", "p.yr"),
                               labels=c(expression(mu[psi~",OI"]),expression(p["OI"])))


sim_range_ignored <- readRDS("all_outputs/sim_range_all_uncensored.rds") %>%
  dplyr::filter(eras != "eras2")

summarised_rmse_sim_range_ig <- sim_range_ignored[term %in% c("mu.psi.yr", "p.yr") & eras == "eras5"][,.(rmse_vals = rmse(true_val, estimate), N = .N), 
                                                                                      by = .(visit_mod, term, eras, prop.visits.same)] %>% 
  mutate(scenario = "sim_range_ig")

summarised_rmse_sim_range_ig$nyr <- factor(summarised_rmse_sim_range_ig$eras, 
                              levels = c("eras2", "eras5", "eras10"), 
                              labels = c(expression("2"~" OIs"),expression("5"~" OIs"),expression("10"~" OIs")))

summarised_rmse_sim_range_ig$term <- factor(summarised_rmse_sim_range_ig$term, 
                               levels=c("mu.psi.yr", "p.yr"),
                               labels=c(expression(mu[psi~",OI"]),expression(p["OI"])))


sim_range_cen <- readRDS("all_outputs/sim_range_all_censored.rds")

summarised_rmse_sim_range_cen <- sim_range_cen[term %in% c("mu.psi.yr", "p.yr") & eras == "eras5"][,.(rmse_vals = rmse(true_val, estimate), N = .N), 
                                                                                                   by = .(visit_mod, term, eras, prop.visits.same)] %>% 
  mutate(scenario = "sim_range_cen")

summarised_rmse_sim_range_cen$nyr <- factor(summarised_rmse_sim_range_cen$eras, 
                              levels = c("eras2", "eras5", "eras10"), 
                              labels = c(expression("2"~" OIs"),expression("5"~" OIs"),expression("10"~" OIs")))

summarised_rmse_sim_range_cen$term <- factor(summarised_rmse_sim_range_cen$term, 
                               levels=c("mu.psi.yr", "p.yr"),
                               labels=c(expression(mu[psi~",OI"]),expression(p["OI"])))


range_together <- bind_rows(summarised_rmse_sim_all, summarised_rmse_sim_range_ig, summarised_rmse_sim_range_cen) %>% 
  mutate(visit_mod = str_remove(visit_mod, "no|yes")) %>% 
  mutate(prop.visits.same = as.numeric(prop.visits.same))

range_together$scenario <- factor(range_together$scenario, 
                              levels=c("sim_all", "sim_range_ig", "sim_range_cen"),
...
summarised_rmse_sim_range_cen <- sim_range_cen[term %in% c("mu.psi.yr", "p.yr") & eras == "eras10"][,.(rmse_vals = rmse(true_val, estimate), N = .N), 
...
summarised_rmse_sim_range_cen$visit_sim <- factor(summarised_rmse_sim_range_cen$mu.v.yr,
                                  levels=c("-0.1", "0", "0.1"),
...
data_text <- data.frame(label = c("a.", "e.", "c.", "b.", "f.", 'd.'),  # Create data for text
                        x = c(-0.04),
                        y = c(0.42)) %>% 
  bind_cols(expand.grid(visit_sim = unique(summarised_rmse_sim_range_cen$visit_sim), term = unique(summarised_rmse_sim_range_cen$term)))


plot_wf_prob_com_visit <-  ggplot() +
  geom_point(data = summarised_rmse_sim_range_cen, aes(x = prop.visits.same, y = rmse_vals, colour = visit_mod, group = visit_mod)) +
  geom_line(data = summarised_rmse_sim_range_cen, aes(x = prop.visits.same, y = rmse_vals, colour = visit_mod, group = visit_mod)) +
