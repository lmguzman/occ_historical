...
summarised_rmse$eras <- factor(summarised_rmse$eras, 
...
data_text <- data.frame(label = c("e.", "a.", "c.", "f.", "b.", 'd.'),  # Create data for text
  ...
  bind_cols(expand.grid(eras = unique(summarised_rmse$eras), term = unique(summarised_rmse$term)))
...
plot_wf_prob_com_nyr <-  ggplot() +
  ...
  facet_grid(eras~term, labeller=label_parsed, scales = "free") +
...
ggsave(plot_wf_prob_com_nyr, filename = "figures_clean/nyr_pcom.pdf", height = 8, width = 9.5)
