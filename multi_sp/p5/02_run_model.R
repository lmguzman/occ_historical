...
file_sim <- list.files("~/scratch/occ_historical/multi_sp/p5/outputs/sim.data/")

run_prep_model <- function(file, case, range, model){
  
  load(paste0("~/scratch/occ_historical/multi_sp/p5/outputs/sim.data/", file), verbose = TRUE)
  #load(paste0("multi_sp/p5/outputs/sim.data/", file), verbose = TRUE)
  ## source model
  ...
  save(res, data.prepped, sim.data,
       file=paste0("~/scratch/occ_historical/multi_sp/p5/outputs/model.res/", case, range, file))
}
