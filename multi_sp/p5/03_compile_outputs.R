...
file_sim <- list.files("~/scratch/occ_historical/multi_sp/p5/outputs/model.res/")
#file_sim <- list.files("multi_sp/5/outputs/model.res/")

compiled_res <- list()

for(i in 1:length(file_sim)){
  
  load(paste0("~/scratch/occ_historical/multi_sp/p5/outputs/model.res/",file_sim[i]), verbose = TRUE)
  #load(paste0("multi_sp/p5/outputs/model.res/",file_sim[i]), verbose = TRUE)
...
saveRDS(all_outputs, 'p5/outputs/model.summary/all_outputs.rds')
