...
file_sim <- list.files("sim_range/outputs/model.res/", pattern="no")

compiled_res <- list()

for(i in 1:length(file_sim)){
  
  load(paste0("sim_range/outputs/model.res/",file_sim[i]), verbose = TRUE)
...
saveRDS(all_outputs, 'sim_range/outputs/model.summary/all_uncensored_outputs_base.rds')
