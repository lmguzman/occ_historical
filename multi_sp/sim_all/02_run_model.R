...
library(stringr)
library(dplyr)
source('~/scratch/occ_historical/analysis/simulation/src/prep_data.R')

file_sim <- list.files("~/scratch/occ_historical/analysis/sim_all/outputs/sim.data/")

run_prep_model <- function(file, case, range, model, time.interval.yr, time.interval.visit=3){
  
  load(paste0("~/scratch/occ_historical/analysis/sim_all/outputs/sim.data/", file), verbose = TRUE)

    ## source model
  source(sprintf('~/scratch/occ_historical/analysis/simulation/models/%s.R', model))
  ...
  save(res, data.prepped, sim.data,
       file=paste0("~/scratch/occ_historical/analysis/sim_all/outputs/model.res/", case, range, "_eras", time.interval.yr, file))
}
