...
file_sim <- list.files("multi_sp/p4/outputs/sim.data/stab_visit/")

run_prep_model <- function(file, case, range, model){
  
  load(paste0("multi_sp/p4/outputs/sim.data/stab_visit/", file), verbose = TRUE)
  ...
  save(res, data.prepped, sim.data,
       file=paste0("multi_sp/p4/outputs/model.res/stab_visit/", case, range, file))
}
