...
file_sim <- list.files("multi_sp/p4/outputs/sim.data/P4.1/")

run_prep_model <- function(file, case, range, model){
  
  load(paste0("multi_sp/p4/outputs/sim.data/P4.1/", file), verbose = TRUE)
  ...
  save(res, data.prepped, sim.data,
       file=paste0("multi_sp/p4/outputs/model.res/P4.1/", case, range, file))
}
