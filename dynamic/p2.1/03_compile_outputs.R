...
file_sim <- list.files("p2.1/outputs/model.res/")

compiled_res <- list()

for(i in 1:length(file_sim)){
  
  load(paste0("p2.1/outputs/model.res/",file_sim[i]), verbose = TRUE)
  ...
  compiled_res[[i]] <- full_join(full_join(summary_mcmc, rhat_mcmc), true_value) %>% 
    mutate(range_sim = file_params[13], range_mod = NA, visit_sim = file_params[16], visit_mod = file_params[1], 
           r = file_params[3], nyr = file_params[11]) %>% 
    data.table()
...
saveRDS(all_outputs, 'p2.1/outputs/model.summary/all_outputs.rds')
